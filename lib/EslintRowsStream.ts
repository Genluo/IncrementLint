...
export default class CompareFile extends Transform {
    ...
    private lint = () => {
        while(this.dataList.length) {
            ...
            const contentList = eslintOutputStr.split('\n').filter(item => item.trim());
    ...
    private dealContent = (contentList: string[], lintRows: number[]) => {
        ...
        `eslint 检测总问题数量 ${lintErrorNum - 3}  ❌  错误总数：${errorNum}  ⚠️  警告总数：${warningNum} ❌  本次修改错误：+${incrementErrorNum}  ⚠️  本次修改警告+${incrementWarningNum}
        `);
